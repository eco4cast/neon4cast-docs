...
Preliminary results will be distributed using the NEON Ecological Forecast Challenge [dashboard](http://shiny3.ecoforecast.org/){target="_blank"} and using the following R code example (where `theme` is the theme you submitted to and `model_id` is your team name)
...
The [dashboard](https://shiny3.ecoforecast.org){target="_blank"} shows the forecast submissions by each team for each forecast theme by date and forecast variable.
