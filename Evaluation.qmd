...
## Forecast Submission Visualization

The [dashboard](https://projects.ecoforecast.org/neon4cast-dashboard){target="_blank"} shows the forecast submissions by each team for each forecast theme by date and forecast variable.

## Evaluation

Preliminary results will be distributed using the `neon4cast::combined_scores(theme)` function (where `theme` is the theme name).
...
## Null forecast {#sec-nulls}

All forecasts will be compared to a null forecast produced by a simple historical-means calculation or a random walk.
