...
Preliminary results will be distributed using the NEON Ecological Forecast Challenge [dashboard](http://shiny3.ecoforecast.org/){target="_blank"} and the `neon4cast::combined_scores()` function (where `x` is an optional argument that is the theme name)
