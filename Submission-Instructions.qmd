...
The following provides the requirements for the format of the forecasts that will be submitted. It is important to follow these format guidelines in order for your submitted forecasts to pass a set of internal checks that allow the forecast to be visualized on the [NEON Ecological Forecast Challenge dashboard](http://shiny3.ecoforecast.org/){target="_blank"} and evaluated with the scoring process.
...
3)  Generate the metadata. Do you best to fill it out but if you are having trouble please submit what you have. Don't let metadata submission get in the way of submitting a forecast.\
4)  Submit (i.e., upload) forecast and metadata.\

## Step 1: Forecast format
...
-   `parameter` required to be the string `mu` or `sigma` (see note below about parameter column).
-   `variable`: standardized variable name from the theme
...
3)  `team_name_ID` is the code for the team name that is specified in the registration (`team_name_ID` has no spaces in it).\
